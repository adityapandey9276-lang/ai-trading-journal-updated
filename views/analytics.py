...
def show(user):
    ...
    with c2:
        st.markdown("<div class='mp-card' style='padding:1rem;'>", unsafe_allow_html=True)
        st.plotly_chart(drawdown_chart(df), use_container_width=True, config={"displayModeBar": False})
        st.markdown("</div>", unsafe_allow_html=True)

    c3, c4 = st.columns(2)
    with c3:
        st.markdown("<div class='mp-card' style='padding:1rem;'>", unsafe_allow_html=True)
        st.plotly_chart(pnl_bar(df, group_by="date", title="Daily PnL"), use_container_width=True, config={"displayModeBar": False})
        st.markdown("</div>", unsafe_allow_html=True)
    with c4:
        st.markdown("<div class='mp-card' style='padding:1rem;'>", unsafe_allow_html=True)
        st.plotly_chart(monthly_performance(df), use_container_width=True, config={"displayModeBar": False})
        st.markdown("</div>", unsafe_allow_html=True)

    # ── Heatmap ──────────────────────────────────────────────
    if df["strategy"].nunique() > 1 and df["symbol"].nunique() > 1:
        st.markdown("<div class='mp-card' style='padding:1rem;'>", unsafe_allow_html=True)
        st.plotly_chart(strategy_heatmap(df), use_container_width=True, config={"displayModeBar": False})
        st.markdown("</div>", unsafe_allow_html=True)
